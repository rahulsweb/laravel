...
<div class="row shop-tracking-status">
    ...
    <div class=" col-md-12">
        <div class="well">
         <h2 class="text-center text-warning"><b>Your Order Status</b></h2>
    <form action="{{ route('order.track') }}" method="POST">
{{ csrf_field() }} 
  
            <div class="form-horizontal">
                <div class="form-group {{ $errors->has('email') ? 'has-error' : ''}}">
                    <label for="inputOrderTrackingID" class="col-sm-2 control-label text-info">Order Email</label>
                        <div class="col-sm-offset-3  col-sm-6">
                        <input type="text" class="form-control" name="email" id="email" value="" placeholder="# put your order email id here">
                 ...
                 <div class="form-group {{ $errors->has('order_code') ? 'has-error' : ''}}">
                    <label for="inputOrderTrackingID" class="col-sm-2 control-label text-info">Order id</label>
                        <div class="col-sm-offset-3 col-sm-6">
                        <input type="text" class="form-control" name="order_code" id="inputOrderTrackingID" value="" placeholder="# put your order id here">
                      {!! $errors->first('order_code', '<strong class="text-danger">:message</strong>') !!}
                    </div>
                       <h6> </h6>
                </div>
                <div class="form-group">
                    <div class="col-md-12 text-center">
                        <button type="submit" id="shopGetOrderStatusID" class="btn btn-success">Order Track</button>
                    </div>
                </div>
            </div>
    </form>
...
<section class="products" >
       ...
       <h5  >Order Code :<b class="text-info pull-right">{{ $product->order_code}}</b></h5> 
     <h5>Payment Type: <b class="text-warning pull-right">{{ $product->orderPayment[0]->payment_type}}</b></h5>
          <h5  >status: <b class="text-danger  pull-right" id="status">{{ $product->orderPayment[0]->status}}</b></h5>
<h5>Date <b class="text-danger  pull-right" >{{$product->log_order->last()->created_at}}</b></h5>

     <h5>Total Qty:<b class=" text-info pull-right">{{ $product->order_carts[0]->pivot->qty}}</b></h5>
<h5>Total:<b class="text-danger  pull-right" >{{ "Rs ".$product->order_carts[0]->pivot->total}}</b></h5>
...
@endforeach
            ...
            <div class="order-status">
                ...
                <div class="image-order-status image-order-status-intransit active img-circle">
                    <span class="status">Dispatch</span>
                    <div class="icon"></div>
                </div>
...
status=$('#status').text();
    ...
    switch(status) {
  case 'Pending':
   $('.order-status-timeline-completion.c3').css("width","0%");
    break;
  case 'Processing':
 $('.order-status-timeline-completion.c3').css("width","22%");
    break;
     case 'Dispatch':
 $('.order-status-timeline-completion.c3').css("width","46%");

    break;
      case 'Shipping':
 $('.order-status-timeline-completion.c3').css("width","70%");

    break;
        case 'Delivered':
 $('.order-status-timeline-completion.c3').css("width","100%");
